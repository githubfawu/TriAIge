...
public sealed class ChallengeUploadServiceTests
{
    ...
    {
        var session = await CreateService().UploadAsync(Json(Envelope(20)), TestContext.Current.CancellationToken);
    ...
    {
        var session = await CreateService().UploadAsync(Json("[" + Records(3, withKeys: true) + "]"), TestContext.Current.CancellationToken);
    ...
    {
        var act = () => CreateService().UploadAsync(Json(input), TestContext.Current.CancellationToken);
    ...
    public async Task UploadAsync_StreamLargerThanLimit_ThrowsFormatException_PerAC4()
    {
        ...
        var act = () => CreateService().UploadAsync(huge, TestContext.Current.CancellationToken);
    ...
    public async Task UploadAsync_StreamThrowingIOException_IsMappedToFormatException_PerAC4()
    {
        ...
        var act = () => CreateService().UploadAsync(broken, TestContext.Current.CancellationToken);
    ...
    public async Task UploadAsync_SameFileTwice_ReportsUnchangedAndLocked_PerAC3()
    {
        ...
        await service.UploadAsync(Json(Envelope(4)), TestContext.Current.CancellationToken);
        _analysis.LockedKeys.Add("#2");

        var second = await service.UploadAsync(Json(Envelope(4)), TestContext.Current.CancellationToken);
    ...
    public async Task UploadAsync_ResolvesIngestorFromNewScopePerUpload()
    {
        ...
        await service.UploadAsync(Json(Envelope(1)), TestContext.Current.CancellationToken);
        await service.UploadAsync(Json(Envelope(1)), TestContext.Current.CancellationToken);
    ...
    public async Task UploadAsync_DuplicateRealKeys_AreReported()
    {
        ...
        var session = await CreateService().UploadAsync(Json(input), TestContext.Current.CancellationToken);
    ...
    public async Task GetProgressAsync_DuplicateIds_CountTotalOverDistinctIds()
    {
        ...
        var session = await service.UploadAsync(Json(input), TestContext.Current.CancellationToken);
        _analysis.IsPending = id => id == 2;

        var progress = await service.GetProgressAsync(session, TestContext.Current.CancellationToken);
    ...
    public async Task GetProgressAsync_AllAnalysed_IsCompleteAndCountsFallbacks_PerAC2()
    {
        ...
        var service = CreateService();
        var session = await service.UploadAsync(Json(Envelope(3)), TestContext.Current.CancellationToken);
    ...
    {
        var service = CreateService();
        var session = await service.UploadAsync(Json(Envelope(2)), TestContext.Current.CancellationToken);
        _analysis.IsPending = _ => true;

        var early = await service.GetProgressAsync(session, TestContext.Current.CancellationToken);
        _time.Advance(TimeSpan.FromSeconds(121));
        var late = await service.GetProgressAsync(session, TestContext.Current.CancellationToken);
    ...
    public async Task GetProgressAsync_LiveWorkerPastWaitTimeout_IsTimedOut_PerAC2()
    {
        ...
        var before = await service.GetProgressAsync(session, TestContext.Current.CancellationToken);
        _time.Advance(TimeSpan.FromSeconds(1200));
        var after = await service.GetProgressAsync(session, TestContext.Current.CancellationToken);
    ...
    public async Task ExportAsync_PendingTicket_UsesFallbackOnce_PerAC2()
    {
        ...
        var export = await service.ExportAsync(session, TestContext.Current.CancellationToken);
    ...
    {
        var service = CreateService();
        var session = await service.UploadAsync(Json(Envelope(20)), TestContext.Current.CancellationToken);
    ...
    public async Task ExportAsync_Bytes_EqualBatchStyleWriterOutput_PerAC5()
    {
        var service = CreateService();
        var raw = Envelope(3);
        var session = await service.UploadAsync(Json(raw), TestContext.Current.CancellationToken);
        ...
        await ChallengeDocument.WriteAsync(
            document.ToOutput([.. export.Rows.Select(r => r.Result)]), expected, TestContext.Current.CancellationToken);
