...
**Resolution-Status:** Feld 6 der Erfolgskriterien (§2) ist **umgesetzt**. Der Core-Port `IResolutionDrafter` liefert `ResolutionDraft(Status, Comment)`, die Pipeline setzt `TriageSuggestion.ResolutionStatus`, der Validator prüft ihn (`InvalidResolutionStatus`), und `TriageResult.Resolution` schreibt ihn in `result.json` im Vokabular der Export-Dateien (`done`, `cancelled`, `clarification`, `cannot reproduce`, kleingeschrieben). Ein ungültiger Modellwert führt zu Retry und Fallback; der Fallback-Status ist der häufigste Status der ähnlichen Tickets, sonst `done`. Die Trainingsdaten enthalten kein Signal für den Status (gemessen, siehe [score-completeness](features/score-completeness/README.md)), deshalb entscheidet das Modell aus dem Text des Tickets.
